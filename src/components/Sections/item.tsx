...
const SectionItem: FC<SectionItemProps> = ( {
  color, index, data, width, points, sections,
} ) => {
  ...
  const animatedProps = useAnimatedProps( () => {
    ...
    const startPoint = points.value[data.value.to.x.indexOf( closestStart )]?.x ?? 0;
    const endPoint = points.value[data.value.to.x.indexOf( closestEnd )]?.x ?? width.value;
